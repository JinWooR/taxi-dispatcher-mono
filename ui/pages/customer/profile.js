...
import { CustomerService } from '../../common/services/CustomerService.js';
 ...
 * @param {CustomerProfile} [profile] - 프로필 정보 (update 모드일 때만)
 */
function setMode(mode, profile = null) {
  ...
  if (mode === 'register') {
    subtitle.textContent = '고객 프로필을 등록하세요';
    nameInput.value = '';
    phoneInput.value = '';
...
async function handleFormSubmit(event) {
  ...
  try {
    ...
    if (pageMode === 'register') {
      await CustomerService.registerProfile(name, phone);
    } else if (pageMode === 'update') {
      await CustomerService.updateProfile(name, phone);
    }

    showMessage('프로필이 저장되었습니다. 권한을 승격 중입니다...', 'success');

    // 2. 권한 승격
    const upgradeResponse = await AuthService.upgradeCustomer();
