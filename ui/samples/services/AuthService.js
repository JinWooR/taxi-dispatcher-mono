...
const AuthService = {
  ...
  upgradeCustomer: async () => {
    const data = await ApiClient.post(`${API_BASE_URL.AUTH}/auth/upgrade/customer`);
    return new LoginResponse(data);
  },
   ...
   */
  upgradeDriver: async () => {
    const data = await ApiClient.post(`${API_BASE_URL.AUTH}/auth/upgrade/driver`);
    return new LoginResponse(data);
  },
...
// const result = await AuthService.register('user@example.com', 'password123', 'CUSTOMER');
