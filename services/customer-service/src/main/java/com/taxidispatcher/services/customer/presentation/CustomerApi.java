 ...
 * Spring MVC 매핑 어노테이션은 CustomerController에서 직접 선언
 */
@Tag(name = "Customer", description = "사용자 프로필 관리")
...
public interface CustomerApi {
    ...
    ResponseEntity<CommonResponse<CustomerProfileResponse>> registerProfile(
    ...
    @ApiResponse(responseCode = "401", description = "인증 실패", content = @Content(schema = @Schema(implementation = CommonResponse.class)))
    ResponseEntity<CommonResponse<CustomerProfileResponse>> getMyProfile(
            @AuthenticationPrincipal AuthUser authUser);
    ...
    @ApiResponse(responseCode = "400", description = "입력값 검증 실패", content = @Content(schema = @Schema(implementation = CommonResponse.class)))
    @ApiResponse(responseCode = "401", description = "인증 실패", content = @Content(schema = @Schema(implementation = CommonResponse.class)))
    ResponseEntity<CommonResponse<CustomerProfileResponse>> updateMyProfile(
            @AuthenticationPrincipal AuthUser authUser,
